...
工作流程（每进入新阶段调用一次 log_progress）：

阶段 A. Profile
  1. 调 profile_csv 获取列元数据、基础统计
  2. 调 sample_rows 抽 5 行看实际内容
  3. 内部总结：这份数据大概是什么业务主题、有哪些关键列

阶段 B. 规划
  4. 根据列类型组合，列出你打算生成的 3–6 张图：
     - 每张图说清 chart_type / x / y / series / 预期用途
     - 不要出现完全重复的图（同样的列、同样的聚合维度）
     - 优先覆盖：整体分布、时间趋势、关键维度对比、相关性（如有数值列对）
  5. 调 log_progress 告知"计划已成型"

阶段 C. 执行
  6. 按计划逐张：
     a. 若需要特殊统计（分布、相关性），先调对应工具（get_column_values / compute_correlation）
     b. 调 render_chart 生成图（传一个合法的 Vega-Lite 规格，包含 data.values）
     c. 调 save_chart_meta 把图的元数据（title/description/relevant_columns/chart_type）落盘
  7. 全部完成后结束对话，不需要文字总结
...
- 所有计算都走工具，不要自己心算
- render_chart 失败（返回 error）时重试不超过 2 次，改用更简单的图替代
...
重要限制：
- **不要调用 profile_csv 和 sample_rows**——列信息已在下方提供。
- 恰好生成 3 张图，不多不少。
...
2. 调 log_progress 告知"计划已成型"。
3. 逐张执行：
   a. 调 get_column_values 获取所需数据。
   b. 调 render_chart 生成图（Vega-Lite spec，包含 data.values）。
   c. 调 save_chart_meta 落盘元数据。
4. 3 张图全部完成后结束对话。
...
铁律：
- 不要贴原始数据、不要心算。
- render_chart 失败重试不超过 1 次，换更简单的图。
- 不要输出自然语言总结。
- 不要询问用户。
- **严格 3 张图，不要生成第 4 张。**
`;
...
阶段 A. 读取输入
  1. 调 read_profile 获取整体数据摘要（行数、列类型、每列的基础统计）
  2. 调 read_chart_meta 获取图表清单（title / description / relevant_columns / chart_type）
...
阶段 C. 总体结论
  4. 调 save_insight({ text, kind: 'summary' }) 写一段 3–5 句的总体结论
     - 数据健康度（缺失、异常）
...
1. 调 read_profile 获取数据摘要。
2. 调 read_chart_meta 获取图表清单。
3. 对每张图：
   a. 如需数据，调 read_column_stats（缓存返回，不重复计算）。
   b. 调 save_insight({ chart_id, text, kind: 'per_chart' }) 写 1–2 句洞察。
      - 必须有具体数字。
      - 不要套话。
4. 调 save_insight({ text, kind: 'summary' }) 写 2–3 句总体结论。
5. 结束对话。
